...
public abstract class JUnitTests {
	...
	protected void checkInfoflow(IInfoflow infoflow, int resultCount) {
		if (infoflow.isResultAvailable()) {
			...
			assertTrue(map.isPathBetweenMethods(sink, sourceDeviceId)
					|| map.isPathBetweenMethods(sink, sourceIMEI) // implicit
																	// flows
	...
	/** 初始化Infoflow，Infoflow是整个污点分析过程的主要Class，缺省参数下是不适用TaintWrapper的*/
	...
	protected IInfoflow initInfoflow(boolean useTaintWrapper) {
		...
		if (useTaintWrapper) {
			...
			try {
				easyWrapper = new EasyTaintWrapper(new File(		//TaintWrapper是根据EasyTaintWrapperSource.txt文件进行设置的
