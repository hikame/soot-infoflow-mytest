...
public class ConstantTests extends JUnitTests {
	...
	@Test(timeout = 300000)
	public void easyConstantFieldTest() {	//验证了对于tainted静态域的传播有效
	...
	@Test(timeout = 300000)
	public void easyConstantVarTest() {		//验证了对于tainted final变量有效（ ）
		IInfoflow infoflow = initInfoflow();
		List<String> epoints = new ArrayList<String>();
	...
	public void fpConstIntraproceduralTest1() {
		...
		infoflow.computeInfoflow(appPath, libPath, epoints, sources, sinks);
		negativeCheckInfoflow(infoflow);	//表示期望infoflow中没有发现sink到source的路径。
	}
	...
	public void fpConstInterproceduralTest6() {
		IInfoflow infoflow = initInfoflow();
		List<String> epoints = new ArrayList<String>();
		epoints.add("<soot.jimple.infoflow.test.ConstantTestCode: void fpConstInterproceduralTest6(int)>");
		infoflow.computeInfoflow(appPath, libPath, epoints, sources, sinks);
		negativeCheckInfoflow(infoflow);
	}
