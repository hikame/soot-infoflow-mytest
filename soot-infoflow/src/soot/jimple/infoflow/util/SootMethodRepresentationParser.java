...
public class SootMethodRepresentationParser {
	 ...
	 * 传入的是诸如“<android.content.Intent: android.content.Intent putExtras(android.os.Bundle)>”的一个方法列表
	 * 对于列表中的每一条语句，解析出方法所属于的类名（如android.content.Intent），以之为Hash表的Key值，Hash表的Value值又是一个列表
	 * 列表中是属于该Key所代表的类中的一些方法，而这些方法正是传入的methods参数中所标明的，如android.content.Intent putExtras(android.os.Bundle)
	 * 注意，这个函数并没有对类、方法是否真实存在进行检验，仅仅是一个字符串的处理分析而已。
	 */
