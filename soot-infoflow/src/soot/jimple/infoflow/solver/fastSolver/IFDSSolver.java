...
public class IFDSSolver<N,D extends FastSolverLinkedNode<D, N>,M,I extends BiDiInterproceduralCFG<N, M>> {
	...
	protected void submitInitialSeeds() {
		Set<Entry<N, Set<D>>>  entrySet = initialSeeds.entrySet();//此处的initialSeeds为所有的source点，也就是我们设置的TargetMethods		
		for(Entry<N, Set<D>> seed: entrySet) {
			N startPoint = seed.getKey();
			Set<D> values = seed.getValue(); 
			for(D val: values)
