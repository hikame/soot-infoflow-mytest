...
public class Options extends OptionsBase {
	//----------------------------------------------------------------------------------------------
	//Created by Kame, 20151124. To dynamically config phantom classes.
	/**作用于SootResolver.bringToHierarchy方法，配合nonPhantomList
	 * 当defaltPhantom为true时，只有nonPhantomList列表中的类会被寻找源码并尝试加载，其他都为Phantom类*/
	private boolean defaltPhantom = false;
	
// 当调用设置setNonPhantomList时自动将defaltPhantom设置为true
//	public void setDefaltPhantom(boolean b){
//		defaltPhantom = b;
//		if(b == true)
//			allow_phantom_refs = true;
//	}
	public boolean getDefaltPhantom(){
		return defaltPhantom;
	}
	
	/**作用于SootResolver.bringToHierarchy方法，配合defaltPhantom
	 * 当defaltPhantom为true时，只有nonPhantomList列表中的类会被寻找源码并尝试加载，其他都为Phantom类*/
    ...
    public List<String> getNonPhantomList() { 
        if( nonPhantomList == null )
            return java.util.Collections.emptyList();
        else
            return nonPhantomList;
    }
	//----------------------------------------------------------------------------------------------
    ...
    /**所有processDir目录下的class文件都要被解析、载入到Body级别的！*/
